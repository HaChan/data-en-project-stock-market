...
ARG dbt_core_ref=dbt-core@v1.7.8
ARG dbt_postgres_ref=dbt-core@v1.7.8
ARG dbt_redshift_ref=dbt-redshift@v1.7.3
ARG dbt_bigquery_ref=dbt-bigquery@v1.7.5
ARG dbt_snowflake_ref=dbt-snowflake@v1.7.2
...
# Update python
RUN python -m pip install --upgrade pip setuptools wheel boto3 --no-cache-dir

# Set docker basics
WORKDIR /usr/app/dbt/
# Removed due to issue with data persisting in this directory if written while building
# an image.
# https://github.com/dbt-labs/dbt-core/issues/4784
# VOLUME /usr/app
ENTRYPOINT ["dbt"]
